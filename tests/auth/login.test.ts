...
test('should pass if it doesn\'t find the user with the email', t => {
  const fakeBody = {
  ...
  UserMock.expects('findOne').withArgs({ email: fakeBody.email }).returns(null);
...
// test('should pass if it fails to validate user password', t => {
//   const fakeBody = {
//     email: fakeUserData.email,
//     password: fakeUserData.password
//   };
...
//   UserMock.expects('findOne').withArgs({ email: fakeBody.email }).returns(fakeUser);
//   FakeUserMock.expects('isValidPassword').withArgs(fakeBody.password).returns(false);
